...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  const updateProduct=await cartService.updateProductInCart(req.user,req.body.productId,req.body.quantity)
...
const checkout = catchAsync(async (req, res) => {
   let checkoutUser=await cartService.checkout(req.user);
   if(checkoutUser===true) res.status(httpStatus.NO_CONTENT).send();
});
